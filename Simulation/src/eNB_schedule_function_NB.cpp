	...
	uint32_t **UL_Channel_bitmap,SIB2_NB * SIB2_NB_S,UL_IND_t & UL_Indicaiton)
{
	...
	list<UE_TEMPLATE_NB> & UE_Info_List=UL_Indicaiton.UL_UE_Info_List;//Get the sche_Info of UL transmissions in UL_UE_list.
	...
	for (it1=UE_Info_List.begin(); it1!=UE_Info_List.end();++it1)
	{
		...
		else if(((*it1).configured==true)&&((*it1).CRC_indication==1))//Ready to rescheudle Msg5, ULInfo except for Msg3
		{
			(*it1).round++;
		}
		// LOG("[NB_schedule_dlsch]:UE_id:%d,round:%d,CE_Level:%d,multi_tone:%d,PHR:%d,DV:%d,UL_Buffer_Size:%d,first_Arrival_Time:%d\n",(*it1).UE_id,(*it1).round,(*it1).CE_Level,(*it1).multi_tone,(*it1).PHR,(*it1).DV,(*it1).UL_Buffer_Size,(*it1).first_Arrival_Time);
		// system("pause");
	}
	//Sort_UEs in DCIs_resource_determinaiton()
	DCIs_resource_determinaiton(scheFrame,scheSubframe,SIB2_NB_S,UE_Info_List,Sche_Response,DL_Channel_bitmap);//DCIs start/end time in search space
	resourceAllocation(SIB2_NB_S,UL_Channel_bitmap,UE_Info_List);//scheduling delay(Idelay)/subcarrier ind(Isc)
