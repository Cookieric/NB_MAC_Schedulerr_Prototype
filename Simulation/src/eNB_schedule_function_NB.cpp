...
void NB_schedule_ulsch(uint32_t scheH_SFN,frame_t scheFrame,sub_frame_t scheSubframe,uint32_t CE_Level,MIB_NB & MIB_NB_S,SIB1_NB & SIB1_NB_S,SIB2_NB & SIB2_NB_S,UL_IND_t & UL_Indicaiton)
{
	...
	if(scheFlag)//CE 0/1/2
	{
		...
		for (it1=UE_Info_List.begin(); it1!=UE_Info_List.end();++it1)
		{
			...
			(*DCI_it1).DCI_Format.DCI_UL_PDU.DCIN0.DCIRep=get_DCI_Filed(T_SearchSpace[CE_Level],DCI_Rep[(*it1).CE_Level]);
		...
		LOG("Sched UEs done in UE_List: wait UL buffersize and shceStatus update when receive DV/BSR\n");
		for (it1=UE_Info_List.begin(); it1!=UE_Info_List.end();++it1)
		{
			if((*it1).schedStatus==true)//ScheduleDone this pp, reset UL Buffer size, it only update when receive DV and BSR
			{
