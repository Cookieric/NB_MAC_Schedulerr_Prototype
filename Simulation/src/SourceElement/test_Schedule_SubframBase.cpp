...
int transfer_PHR(UL_UE_list &, int);
int transfer_MCS(UL_UE_list &);
int transfer_DV(UL_UE_list &);
int get_UL_Data_Bytes(int);

int resourceAllocate(UL_UE_list &);
...
int preSubframeMapping(int, UL_UE_list &,NB_SIB2 &,int **);
// bool lock=false;
...
int main(int argc, char const *argv[])
{
 ...
 //    	// printf("strerror(errno)=%s\n", strerror(errno));
	...
	list<UL_UE_list> UL_Sche_UE_List;
	list<UL_UE_list>::iterator it1;
	...
	int pcmax=23;
	int measuredpuschpower=0;//Measured quantity value (dB) in R4-1610577
	int numUE=10;
	int UE_id=10;
	for (int i = 0; i < numUE; ++i)	UL_Sche_UE_List.push_back (UL_Sche_UE);
	...
	// }
	cout<<"subframe Mapping for a specific size done..."<<endl;
	cout<<endl;
...
int preSubframeMapping(int subframe, UL_UE_list &UL_Sche_UE,NB_SIB2 &NB_SIB2_Struc,int ** UL_Channle_Struc)
{
	...
	{
		int skipTTI=ceil(1.4 * 4 * NB_SIB2_Struc.rep[0]);//Skip CE 0 NPRACH resource
...
int resourceAllocate(UL_UE_list &UL_Sche_UE)
{
	int RU_unit=1;
...
int transfer_MCS(UL_UE_list &UL_Sche_UE)
{
	if(UL_Sche_UE.PHR==0)
...
int transfer_PHR(UL_UE_list &UL_Sche_UE, int measuredpuschpower)
{
...
int transfer_DV(UL_UE_list &UL_Sche_UE)
{
